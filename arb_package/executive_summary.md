...
**M&T Bank | Cari Deposit Account (CDA) Platform on Cari Network**
...
M&T Bank proposes to launch FDIC-insured Cari Deposit Accounts (CDA) on the Cari Network, a permissioned blockchain built on ZKsync Prividium (zkRollup Layer 2). This initiative enables M&T to offer programmable, instantly-settleable bank deposits to institutional and commercial clients, with full GENIUS Act compliance, FDIC insurance coverage, and OCC examiner transparency.

The platform implements a **dual-rail architecture** where CDA tokens (on-chain) operate in parallel with traditional Demand Deposit Accounts (DDA) off-chain, with the **Operator** (M&T Bank) controlling CDA supply through mint/burn operations, and the **Settlement Bank** executing daily net settlement of inter-bank CDA transfers via the **Messaging Bridge**.

### M&T Bank Technology Stack Alignment
...
| **Container Registry** | Azure ACR (mtbcari.azurecr.io) | Docker image storage and scanning |
...
| Driver | Detail |
|--------|--------|
| **Market Opportunity** | $2.1T tokenized asset market by 2030 (BCG estimate); M&T targets $500M-$1B CDA AUC by 2028 |
...
| **Strategic Positioning** | M&T is a Cari Network founding partner; first-mover advantage among regional banks |
...
| Supply Control | Operator Role | M&T Bank as centralized CDA supply controller (mint/burn) |
...
| Off-Chain Framework | FastAPI (Python) | Async performance; Pydantic validation; M&T team familiarity |
| Container Platform | Azure AKS | M&T standard; native Azure integration; RBAC |
| Container Registry | Azure ACR (mtbcari.azurecr.io) | Private registry; vulnerability scanning; geo-replication |
| Custody | Fireblocks MPC (primary) | SOC 2 Type II; 1800+ institutional clients; MPC-CMP |
| AML/OFAC | Chainalysis KYT | Regulatory gold standard; M&T existing relationship |
...
| Event Bus | Kafka (Confluent KRaft) | High-throughput event streaming; M&T standard |
...
4. UAT sign-off from M&T Treasury Operations
