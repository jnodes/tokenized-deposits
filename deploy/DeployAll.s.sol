...
/**
 * @title DeployAll
 * @notice Deployment script for Cari Deposit Account (CDA) contracts on ZKsync Prividium.
 *
 *         TERMINOLOGY:
 ...
 *         2. TokenizedDeposit (behind ERC1967 proxy, linked to oracle) - CDA token contract
 ...
 *           controlled by a consortium multi-sig (e.g., Gnosis Safe with 3/5 threshold).
...
contract DeployAll is Script {
    ...
    //                 Replace with consortium multi-sig addresses
    // =========================================================================

    // Consortium Timelock/multi-sig (DEFAULT_ADMIN_ROLE on all contracts)
    ...
    // HSM-backed minter (treasury operations)
    address constant MINTER = 0x3333333333333333333333333333333333333333;

    // HSM-backed burner (redemption service)
    ...
    function run() external {
        ...
        // --- 2. Deploy TokenizedDeposit (CDA token contract) ---
        TokenizedDeposit tokenImpl = new TokenizedDeposit();
        ERC1967Proxy tokenProxy = new ERC1967Proxy(
            address(tokenImpl),
            abi.encodeCall(TokenizedDeposit.initialize, (ADMIN, address(oracle)))
        );
        TokenizedDeposit token = TokenizedDeposit(address(tokenProxy));
        console.log("TokenizedDeposit proxy:", address(token));
        console.log("TokenizedDeposit impl: ", address(tokenImpl));
        ...
        console.log("Bank:    Issuing Bank");
        console.log("");
        console.log("NEXT STEPS (via consortium multi-sig):");
